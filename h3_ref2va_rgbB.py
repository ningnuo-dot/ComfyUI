#!/usr/bin/env python3
"""MiniMax H3 Ref2VA 第二臂（原 RGB 视频参考）运行脚本

实验 B（本脚本）：四视图 + 米兰场景 + 原 RGB 视频(24fps 帧序列) → 与深度臂同参数出片对比
实验 A（h3_ref2va_depthA.py）：同参数换深度视频参考，A/B 对比深度 vs RGB 参考

参考接线（MiniMaxH3ReferenceToVideo）：<Picture 1>=四视图 <Picture 2>=米兰场景 <Video 1>=RGB 动作帧序列
"""
import json
import time
...
def main():
    ...
    g = {
        ...
        "6": {"class_type": "LoadVideo", "inputs": {"file": "h3_rgb_ref_24fps.mp4"}},
        ...
        "18": {"class_type": "SaveVideo", "inputs": {
            "video": ["17", 0], "filename_prefix": "video/MiniMax_H3/ref2va_rgbB",
